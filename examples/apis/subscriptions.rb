class Application < Sinatra::Application
  ...
  get '/v1/customers/:customer_id/subscriptions' do
    subscriptions = client.customers_subscriptions.with(params[:customer_id]).all
    JSON.pretty_generate(subscriptions.attributes)
  end

  get '/v1/customers/:customer_id/subscriptions/:id' do
    payment = client.customers_subscriptions.with(params[:customer_id]).get(params[:id])
    JSON.pretty_generate(payment.attributes)
  end
  ...
  delete '/v1/customers/:customer_id/subscriptions/:id' do
    client.customers_subscriptions.with(params[:customer_id]).delete(params[:id])
    "deleted"
  end
end
